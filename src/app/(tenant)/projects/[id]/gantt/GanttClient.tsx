...
export function GanttClient({ tasks: initialTasks }: { tasks: GanttTaskInput[] }) {
  ...
  return (
    <div className="container" style={{ maxWidth: "none" }}>
      <h1 style={{ marginBottom: "var(--space-5)" }}>Gantt</h1>
      {tasks.length === 0 ? (
        <div className="empty-state">
          <h3>Keine Tasks mit Fälligkeitsdatum</h3>
        </div>
      ) : (
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div style={{ width: "200px", flexShrink: 0 }}>
            ...
            {tasks.map((task) => (
              <div
                key={task.id}
                style={{ height: `${ROW_HEIGHT}px`, display: "flex", alignItems: "center", fontSize: "var(--text-sm)" }}
              >
                {task.title}
              </div>
            ))}
          ...
          <div style={{ overflowX: "auto", flex: 1 }}>
            <div
              ...
              style={{
                position: "relative",
                width: `${totalDays * PX_PER_DAY}px`,
                height: `${24 + tasks.length * ROW_HEIGHT}px`,
              }}
            >
              <svg
              ...
              {tasks.map((task, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      onMouseDown={(event) => startDrag(task.id, "resize-left", event)}
                      style={{ width: "6px", cursor: "col-resize" }}
                    />
                    <div
                      onMouseDown={(event) => startDrag(task.id, "move", event)}
                      style={{ flex: 1, cursor: "grab" }}
                    />
                    <div
                      onMouseDown={(event) => startDrag(task.id, "resize-right", event)}
                      style={{ width: "6px", cursor: "col-resize" }}
                    />
                  </div>
                );
