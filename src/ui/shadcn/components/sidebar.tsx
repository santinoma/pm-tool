...
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@/ui/shadcn/components/sheet";
import { Skeleton } from "@/ui/shadcn/components/skeleton";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/ui/shadcn/components/tooltip";
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      if (typeof window !== "undefined") {
        window.localStorage.setItem("sidebar-collapsed", JSON.stringify(!openState));
...
export function Sidebar({
  side = "left",
  // eslint-disable-next-line @typescript-eslint/no-unused-vars -- kept for API parity with the original template
  variant = "sidebar",
  collapsible = "icon",
  className,
  children,
  ...
  if (collapsible === "none") {
    return (
      <div data-slot="sidebar" className={cn("flex h-full w-64 flex-col border-e border-border bg-background", className)} {...props}>
        {children}
      </div>
    );
  }
...
export function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-8 w-8 text-muted-foreground hover:bg-accent/40 hover:text-foreground", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
export function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return <main data-slot="sidebar-inset" className={cn("relative flex min-h-0 flex-1 flex-col overflow-hidden", className)} {...props} />;
}
...
export function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("flex h-app-header min-h-app-header w-full min-w-0 shrink-0 items-center overflow-hidden px-4", className)}
      {...props}
    />
  );
}
...
export function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn("mt-auto flex flex-col gap-1 px-4 py-2 group-data-[state=collapsed]:px-2", className)}
      {...props}
    />
  );
}
...
export function SidebarGroup({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="sidebar-group" data-sidebar="group" className={cn("relative flex w-full min-w-0 flex-col py-2", className)} {...props} />;
}
...
export function SidebarMenuItem({ className, ...props }: React.ComponentProps<"li">) {
  return <li data-slot="sidebar-menu-item" data-sidebar="menu-item" className={cn("group/menu-item relative w-full", className)} {...props} />;
}
  ...
  "flex h-10 w-full max-w-full items-center justify-start gap-2.5 overflow-hidden rounded-sm px-[15px] py-2 text-sm font-medium text-sidebar-foreground/60 transition-colors hover:bg-primary/10 hover:text-sidebar-foreground data-[active=true]:bg-primary/10 data-[active=true]:text-primary data-[active=true]:hover:bg-primary/10 data-[active=true]:hover:text-primary",
  {
    variants: {
      variant: {
        ...
        destructive: "hover:bg-destructive/10 hover:text-destructive text-sidebar-foreground/60",
      },
      size: {
...
export function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return <ul data-slot="sidebar-menu-sub" data-sidebar="menu-sub" className={cn("flex min-w-0 flex-col px-1.5 py-2", className)} {...props} />;
}
