...
function SheetHeader({ className, children, showBackButton = false, ...props }: SheetHeaderProps) {
  return (
    <div data-slot="sheet-header" className={cn("flex shrink-0 flex-col gap-1.5 border-b px-6 py-4 pr-14 text-left", className)} {...props}>
      {showBackButton ? (
        <SheetPrimitive.Close
...
function SheetBody({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="sheet-body" className={cn("flex-1 overflow-y-auto px-6 py-4", className)} {...props} />;
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("mt-auto flex shrink-0 flex-col-reverse gap-2 border-t px-6 py-4 sm:flex-row sm:justify-end", className)}
      {...props}
    />
  );
}
